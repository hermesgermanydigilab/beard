...
trait TemplateCompiler {
  def compile(templateName: TemplateName, yieldStatements: Seq[Statement] = Seq.empty, contentForStatements: Map[Identifier, Seq[Statement]] = Map.empty): Try[BeardTemplate]
}
...
class CustomizableTemplateCompiler(
  ...
  def compile(
    ...
    beardTemplateTry.flatMap { beardTemplate =>
      val newContentForStatementsMap = addContentForStatementsToMap(contentForStatementsMap, beardTemplate.contentForStatements)
   ...
   * @param statements initial statements
    ...
    */
  @tailrec
  final private[renderer] def concatTexts(statements: Seq[Statement], mergedStatements: Seq[Statement] = Seq.empty, existingTexts: Seq[HasText] = Seq.empty): Seq[Statement] =
    statements match {
      ...
      case (head: IfStatement) :: tail => {
        val concatTextsIfStatement = head.copy(head.condition, ifStatements = concatTextsRec(head.ifStatements), elseStatements = concatTextsRec(head.elseStatements))
      ...
      case (head: UnlessStatement) :: tail =>
        val concatTextsUnlessStatement = head.copy(head.condition, unlessStatements = concatTextsRec(head.unlessStatements), elseStatements = concatTextsRec(head.elseStatements))
  ...
  private[renderer] def concatTextsRec(statements: Seq[Statement], mergedStatements: Seq[Statement] = Seq.empty, existingTexts: Seq[HasText] = Seq.empty): Seq[Statement] = {
   ...
   * @return a new BeardTemplate with the replaced statements
    */
  private def createMergedTemplate(beardTemplate: BeardTemplate, yieldStatements: Seq[Statement], contentForStatementsMap: Map[Identifier, Seq[Statement]]): BeardTemplate = {
    ...
    val yieldedBeardTemplate = BeardTemplate(newStatements, beardTemplate.extended, beardTemplate.renderStatements, beardTemplate.contentForStatements)
    yieldedBeardTemplate
  }
}

object DefaultTemplateCompiler extends CustomizableTemplateCompiler(new ClasspathTemplateLoader(), new BeardTemplateCache(), new BeardTemplateParser())
