...
/**
  * @author dpersa
  */
class BeardTemplateRenderer(templateCompiler: TemplateCompiler, filters: Seq[Filter] = Seq(), filterResolver: FilterResolver = DefaultFilterResolver()) {
  ...
  private def stringRepresentation(value: Any, escapeStrategy: EscapeStrategy): String =
    value match {
    ...
    statement match {
      ...
      case ForStatement(templateIterator, templateIndex, collection, statements, addNewLine) => {
        ...
        for {
          ...
          renderStatement(statement, ForContextFactory.create(forIterationContext), renderResult, yieldedStatement, escapeStrategy, locale, resourceBundleName)
        }
      }
   ...
   * @param identifierValue: the actual value
    ...
    */
  private[this] def filter[T](identifierValue: Any, filterNodes: Seq[FilterNode], context: Map[String, Any], locale: Locale, resourceBundleName: String): Any = {
